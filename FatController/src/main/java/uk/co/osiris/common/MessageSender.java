...
public class MessageSender {
	...
	public ArrayList<Sensor> getSensors(Node sensors) {
		if (isReachable(sensors)) {
			String url = buildURL(sensors, "sensors");
			ResponseEntity<Sensor[]> response = restTemplate.getForEntity(url, Sensor[].class);
			Sensor[] sensorLists = response.getBody();
			return new ArrayList<>(Arrays.asList(sensorLists));
		}
		return null;
	}
}
